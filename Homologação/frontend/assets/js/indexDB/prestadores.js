async function inicializarPrestadores() {
    ...
    if (form) {
        form.addEventListener("submit", async (e) => {
            ...
            try {
                const response = await fetchComAuth("htpp://localhost:8080/index/prestadores", {
                    method: "POST",
...
async function preencherSelectComPrestadores(elementoSelect) {
    ...
    const urlPrestadores = "htpp://localhost:8080/index/prestadores";
...
function configurarModalDesativarPrestador() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const prestadoresResponse = await fetchComAuth("htpp://localhost:8080/index/prestadores/ativos");
            ...
            const response = await fetchComAuth(`htpp://localhost:8080/index/prestadores/desativar/${prestadorCodigo}`, {
                method: 'PUT',
            });
            // --- FIM DA CORREÇÃO ---
...
async function preencherSelectComPrestadoresDesativados(elementoSelect) {
    const urlPrestadoresDesativados = "htpp://localhost:8080/index/prestadores/desativados";
...
function configurarModalAtivarPrestador() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const prestadoresResponse = await fetchComAuth("htpp://localhost:8080/index/prestadores/desativados");
            const prestadores = await prestadoresResponse.json();
            const prestadorSelecionado = prestadores.find(p => p.id == prestadorId);
            ...
            const response = await fetchComAuth(`htpp://localhost:8080/index/prestadores/ativar/${prestadorCodigo}`, {
                method: 'PUT',
            });
            // --- FIM DA CORREÇÃO ---
...
function configurarModalEditarPrestador() {
    ...
    formEl.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetchComAuth(`htpp://localhost:8080/index/prestadores/${prestadorId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
