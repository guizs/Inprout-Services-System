...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (modalCriarLPU) {
        ...
        formCriarLPU.addEventListener('submit', async function (event) {
            ...
            try {
                const response = await fetchComAuth('http://localhost:8080/lpu', {
                    method: 'POST',
    ...
    if (modalAlterarLPU) {
        ...
        document.getElementById('selectLPUAlterar').addEventListener('change', async function (event) {
            ...
            try {
                ...
                const lpuResponse = await fetchComAuth(`http://localhost:8080/lpu/${lpuId}`);
                ...
                const contratosResponse = await fetchComAuth('http://localhost:8080/contrato');
        ...
        formAlterarLPU.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                const response = await fetchComAuth(`http://localhost:8080/lpu/${lpuId}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
