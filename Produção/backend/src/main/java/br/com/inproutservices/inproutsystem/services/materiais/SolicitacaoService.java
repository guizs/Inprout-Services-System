...
public class SolicitacaoService {
    ...
    public List<Solicitacao> listarHistorico(Long usuarioId) {
        Usuario usuario = usuarioRepository.findById(usuarioId)
                .orElseThrow(() -> new ResourceNotFoundException("Usuário não encontrado com o ID: " + usuarioId));

        Role role = usuario.getRole();

        // Perfis que podem ver todas as solicitações de todos os segmentos
        if (role == Role.ADMIN || role == Role.CONTROLLER || role == Role.ASSISTANT) {
            return solicitacaoRepository.findAll();
        }

        // Gestor e Coordenador veem apenas as solicitações de seus segmentos
        if (role == Role.MANAGER || role == Role.COORDINATOR) {
            ...
            if (segmentosDoUsuario.isEmpty()) {
                return List.of(); // Retorna lista vazia se não tiver segmentos
            }
            // Usa o novo método do repositório
            return solicitacaoRepository.findByOsSegmentoIn(segmentosDoUsuario);
        }

        // Retorno padrão para outros perfis (se houver)
        return List.of();
    }
