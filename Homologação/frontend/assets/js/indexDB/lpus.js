...
async function carregarContratos(selectId, selectedId = null) {
    ...
    try {
        const response = await fetchComAuth('http://3.128.248.3/contrato');
...
async function carregarLpusNoSelect(selectId, filtroAtivo, textoPlaceholder) {
    ...
    const url = `http://3.128.248.3/contrato`;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (modalCriarLPU) {
        ...
        formCriarLPU.addEventListener('submit', async function (event) {
            ...
            try {
                const response = await fetchComAuth('http://3.128.248.3/lpu', {
                    method: 'POST',
    ...
    if (modalAlterarLPU) {
        ...
        document.getElementById('selectLPUAlterar').addEventListener('change', async function (event) {
            ...
            try {
                ...
                const lpuResponse = await fetchComAuth(`http://3.128.248.3/lpu/${lpuId}`);
                ...
                const contratosResponse = await fetchComAuth('http://3.128.248.3/contrato');
        ...
        formAlterarLPU.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                const response = await fetchComAuth(`http://3.128.248.3/lpu/${lpuId}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
    ...
    if (modalAtivarLPU) {
        ...
        btnConfirmarReativacao.addEventListener('click', async function (event) {
            ...
            try {
                const url = `http://3.128.248.3/lpu/${lpuId}`;
                const response = await fetchComAuth(url, {
                    method: 'PATCH',
                    headers: {
