    ...
    */
    ...
    btnAvancarParaPreenchimentoLote.addEventListener('click', async () => {
        ...
        try {
            if (todasAsEtapasLote.length === 0) {
                todasAsEtapasLote = await fetchComAuth('htpp://localhost:8080/index/etapas').then(res => res.json());
            }
            if (todosOsPrestadoresLote.length === 0) {
                todosOsPrestadoresLote = await fetchComAuth('htpp://localhost:8080/index/prestadores/ativos').then(res => res.json());
            }
    ...
    async function handleFormSubmitLote(acao, submitButton) {
        ...
        try {
            ...
            const response = await fetchComAuth('htpp://localhost:8080/lancamentos/lote', {
                method: 'POST',
