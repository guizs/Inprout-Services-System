document.addEventListener('DOMContentLoaded', () => {
    ...
    async function carregarLancamentos() {
        toggleLoader(true);
        try {
            const response = await fetchComAuth('htpp://localhost:8080/lancamentos');
    ...
    if (modalAdicionarEl) {
        ...
        formAdicionar.addEventListener('submit', async (e) => {
            ...
            const url = editingId ? `htpp://localhost:8080/lancamentos/${editingId}` : 'htpp://localhost:8080/lancamentos';
        ...
        async function carregarEPopularLPU(osId) {
            ...
            try {
                ...
                const response = await fetchComAuth(`htpp://localhost:8080/os/${osId}`);
        ...
        async function carregarDadosParaModal() {
            if (todasAsOS.length === 0) {
                try {
                    ...
                    const response = await fetchComAuth(`htpp://localhost:8080/os/por-usuario/${usuarioId}`);
            ...
            if (!todosOsPrestadores || todosOsPrestadores.length === 0) {
                todosOsPrestadores = await popularSelect(selectPrestador, 'htpp://localhost:8080/index/prestadores/ativos', 'id', item => `${item.codigoPrestador} - ${item.prestador}`);
            }
            if (todasAsEtapas.length === 0) {
                todasAsEtapas = await popularSelect(selectEtapaGeral, 'htpp://localhost:8080/index/etapas', 'id', item => `${item.codigo} - ${item.nome}`);
            }
        }
        ...
        async function abrirModalParaEdicao(lancamento, editingId) {
            ...
            if (lancamento.os) {
                ...
                try {
                    const response = await fetchComAuth(`htpp://localhost:8080/os/${lancamento.os.id}`);
            ...
            if (selectPrestadorEl) {
                ...
                const prestadores = await fetchComAuth('htpp://localhost:8080/index/prestadores/ativos').then(res => res.json());
        ...
        document.getElementById('btnConfirmarSubmissao').addEventListener('click', async function (e) {
            ...
            try {
                ...
                const resposta = await fetchComAuth(`htpp://localhost:8080/lancamentos/${id}/submeter`, { method: 'POST' });
                if (!resposta.ok) {
    ...
    if (modalSolicitarMaterialEl) {
        ...
        const popularSelectMateriais = (selectElement) => {
            ...
            if (todosOsMateriais.length === 0) {
                fetchComAuth('htpp://localhost:8080/materiais')
                    .then(res => res.json())
                    .then(data => {
        ...
        selectOS.addEventListener('change', async (e) => {
            ...
            try {
                const response = await fetchComAuth(`htpp://localhost:8080/os/${osId}/lpus`);
        ...
        formSolicitacao.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetchComAuth('htpp://localhost:8080/solicitacoes', {
                    method: 'POST',
