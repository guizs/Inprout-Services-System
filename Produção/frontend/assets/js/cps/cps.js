document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderLancamentosTable(lancamentos) {
        // Pega a role do usuário e define se as ações devem ser exibidas
        const userRole = (localStorage.getItem("role") || "").trim().toUpperCase();
        const podeExibirAcoes = ['ADMIN', 'CONTROLLER', 'ASSISTANT'].includes(userRole);

        // Constrói o cabeçalho da tabela, adicionando a coluna "AÇÕES" apenas se permitido
        let cabecalhoHtml = `
        <tr>
            <th>DATA ATIVIDADE</th> <th>OS</th> <th>SITE</th> <th>CONTRATO</th> <th>SEGMENTO</th>
            <th>PROJETO</th> <th>GESTOR TIM</th> <th>REGIONAL</th> <th>LPU</th> <th>LOTE</th> <th>BOQ</th> <th>PO</th>
            <th>ITEM</th> <th>OBJETO CONTRATADO</th> <th>UNIDADE</th> <th>QTD</th> <th>VALOR TOTAL</th>
            <th>OBSERVAÇÕES</th> <th>DATA PO</th> <th>VISTORIA</th> <th>PLANO DE VISTORIA</th>
            ...
            ${podeExibirAcoes ? '<th>AÇÕES</th>' : ''}
        </tr>`;
        tableHead.innerHTML = cabecalhoHtml;

        tableBody.innerHTML = '';

        if (lancamentos && lancamentos.length > 0) {
            lancamentos.forEach(lanc => {
                const lpuCode = (lanc.lpu || 'N/A').split(' - ')[0];
                const lpuName = lanc.objetoContratado || (lanc.lpu ? lanc.lpu.split(' - ').slice(1).join(' - ') : 'N/A');

                // Cria o HTML dos botões de ação apenas se permitido
                const acoesHtml = podeExibirAcoes ? `
                <td>
            ...
            <tr>
                ...
                <td>${lanc.gestorTim || 'N/A'}</td> <td>${lanc.regional || 'N/A'}</td> <td>${lpuCode}</td> <td>${lanc.lote || 'N/A'}</td>
                <td>${lanc.boq || 'N/A'}</td> <td>${lanc.po || 'N/A'}</td> <td>${lanc.item || 'N/A'}</td>
                <td>${lpuName}</td> <td>${lanc.unidade || 'N/A'}</td> <td>${lanc.quantidade || 'N/A'}</td>
            ...
            // Ajusta o número de colunas na mensagem de "nenhum lançamento"
            const colspan = podeExibirAcoes ? 38 : 37;
