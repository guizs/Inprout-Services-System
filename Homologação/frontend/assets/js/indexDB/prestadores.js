async function inicializarPrestadores() {
    ...
    if (form) {
        form.addEventListener("submit", async (e) => {
            ...
            try {
                const response = await fetchComAuth("http://3.128.248.3/index/prestadores", {
                    method: "POST",
...
async function preencherSelectComPrestadores(elementoSelect) {
    ...
    const urlPrestadores = "http://3.128.248.3/index/prestadores";
...
function configurarModalDesativarPrestador() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const prestadoresResponse = await fetchComAuth("http://3.128.248.3/index/prestadores/ativos");
            ...
            const response = await fetchComAuth(`http://3.128.248.3/index/prestadores/desativar/${prestadorCodigo}`, {
                method: 'PUT',
            });
            // --- FIM DA CORREÇÃO ---
...
async function preencherSelectComPrestadoresDesativados(elementoSelect) {
    const urlPrestadoresDesativados = "http://3.128.248.3/index/prestadores/desativados";
...
function configurarModalAtivarPrestador() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const prestadoresResponse = await fetchComAuth("http://3.128.248.3/index/prestadores/desativados");
            const prestadores = await prestadoresResponse.json();
            const prestadorSelecionado = prestadores.find(p => p.id == prestadorId);
            ...
            const response = await fetchComAuth(`http://3.128.248.3/index/prestadores/ativar/${prestadorCodigo}`, {
                method: 'PUT',
            });
            // --- FIM DA CORREÇÃO ---
...
function configurarModalEditarPrestador() {
    ...
    formEl.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetchComAuth(`http://3.128.248.3/index/prestadores/${prestadorId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
