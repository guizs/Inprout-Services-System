    ...
    if (btnSalvarAlteracoes) {
        btnSalvarAlteracoes.addEventListener('click', async () => {
            ...
            if (novoEmail) {
                ...
                try {
                    const response = await fetchComAuth(`https://www.inproutservices.com.br/api/usuarios/email?emailAtual=${encodeURIComponent(emailAtual)}&novoEmail=${encodeURIComponent(novoEmail)}`, { method: 'PUT' });
                    if (!response.ok) {
